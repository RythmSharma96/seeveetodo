...
const Tasks = ({task, toggleComplete}) => {
  return (
    <motion.h1 
    animate={{x: 0, y: -5}}
    >
    <input className="hidden" type="checkbox" id={task.id} defaultChecked={task.completed} />
    <label onClick={ () => toggleComplete(task.id) } className="flex items-center h-10 px-2 rounded cursor-pointer hover:bg-gray-900" htmlFor={task.id}>
